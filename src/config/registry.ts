...
export function validateRegistryDrift(config: BetaRuntimeConfig) {
  ...
  if (
    ...
    if (config.profile === "production" || !config.contract.registryContractId.startsWith("CAAAA")) {
        throw new Error(
  ...
  console.log(`[Registry] Successfully validated runtime config against signed deployment manifest (Deployed At: ${manifest.deployedAt})`);
}
